...
def apply_fourier_transform(data):
    ...
    # 这里只保留前半部分的频率成分，扔掉高频部分
    transformed_data = transformed_data[:data.shape[0]//2, :]
    # 返回傅立叶变换后的实部和虚部
    return np.real(transformed_data), np.imag(transformed_data)
...
def main(config):
    ...
    # Apply Fourier transform to x_data
    x_real, x_imag = apply_fourier_transform(x_data)
    x_data = np.concatenate((x_real, x_imag), axis=1)
    ...
    # Apply Fourier transform to each slice of x_data
    x_data_slices_real = []
    x_data_slices_imag = []
    for slice in x_data_slices:
        real, imag = apply_fourier_transform(slice)
        x_data_slices_real.append(real)
        x_data_slices_imag.append(imag)
    x_data_slices = np.concatenate((x_data_slices_real, x_data_slices_imag), axis=2)
